    ...
    private class process_thread extends Thread{
        ...
        public void run(){
            ...
            FOR_LOOP: for (int j = 0; j < tag_squ_arr.length; j++) { // v
                ...
                if(chunk_last_verb(/*tag_squ_arr[j] /*a_tag_squ_arr*/ j)){
                   // break FOR_LOOP;
                //}
            ...
            if(corret_tag_sequences.size() > 0){
                for (int i = 0; i < corret_tag_sequences.size(); i++) {
                    my_consol.write(corret_tag_sequences.get(i) + " is currently a correct tag sequece ~~~~~~~~");
                }
        ...
        private void process_fully_cunkable_tag_squs(ArrayList<Integer> corret_tag_sequences){
            ...
            my_consol.write(corret_tag_sequences.size() + " corret_tag_sequences");
            ...
            for (int i = 0; i < corret_tag_sequences.size(); i++) {
                ...
                chunked_words.add(words.get(0));

                for (int j = 1; j < tag_squ_arr[corret_tag_sequences.get(i)].length - 1; j++) {
                ...
                chunked_words.add(words.get(tag_squ_arr[corret_tag_sequences.get(i)].length-1));
        ...
        private boolean chunk_noun(int row_num_tag_seq){
            //my_consol.write("----------------------------------------");
        ...
        private boolean chunk_last_verb(int row_num_tag_seq){
